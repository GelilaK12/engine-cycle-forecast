...
pd.DataFrame([search.best_params_]).to_csv(
    os.path.join(metrics_dir, "rf_best_params.csv"), index=False
)
...
for fold, (train_idx, test_idx) in enumerate(kf.split(X), start=1):
    ...
    # store predictions for overall outliers
    ...
    outliers_path = os.path.join(metrics_dir, f"rul_outliers_fold_{fold}.csv")
...
overall_outliers_path = os.path.join(metrics_dir, "rul_outliers.csv")
...
importances.to_csv(os.path.join(metrics_dir, "feature_importance.csv"))
...
cycle_feature = "cycle"
top_sensor_features = [f for f in features_for_pdp if f.startswith("sensor_")][:3]
pdp_features = [cycle_feature] + top_sensor_features

fig, ax = plt.subplots(figsize=(12,6))
PartialDependenceDisplay.from_estimator(best_rf, X, features=['cycle'], ax=ax)
ax.set_ylabel("Predicted Remaining Useful Life (RUL)")
ax.set_xlabel("Cycle")
ax.set_title("PDP: Cycle vs Predicted RUL")
plt.tight_layout()
save_plot(fig, "pdp_cycle.png", figures_path)
plt.close()

# -----------------------------
# CONFIDENCE INTERVAL + PRIORITIZATION
# -----------------------------
predictions_per_tree = np.stack([tree.predict(X) for tree in best_rf.estimators_], axis=0)
lower = np.percentile(predictions_per_tree, 2.5, axis=0)
upper = np.percentile(predictions_per_tree, 97.5, axis=0)
...
ci_df["violation_magnitude"] = (ci_df["actual_rul"] - ci_df["pred_rul"]).abs()
ci_df["outside_ci"] = (ci_df["actual_rul"] < ci_df["lower_ci"]) | (ci_df["actual_rul"] > ci_df["upper_ci"])

outlier_engines = ci_df[ci_df["outside_ci"]].sort_values(by="violation_magnitude", ascending=False)
outlier_engines.to_csv(os.path.join(metrics_dir, "outlier_engines_priority.csv"), index=False)

engine_priority = outlier_engines.groupby("unit")["violation_magnitude"].sum().reset_index()
engine_priority = engine_priority.sort_values(by="violation_magnitude", ascending=False)
engine_priority.to_csv(os.path.join(metrics_dir, "outlier_engines_aggregated.csv"), index=False)
...
>>>>>>> 4e64612b5a1b80f779d12e60981dfd809cc4272e:scripts/02_train_model.py
